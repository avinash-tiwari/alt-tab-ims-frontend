...
function renderItemRows(items = []) {
  return items.map((item) => `
    <tr>
      <td style="padding: 8px 6px; color: #0f172a; font-weight: 600; vertical-align: top; width: 44%; word-break: break-word;">
      ...
      <td style="padding: 8px 6px; text-align: right; color: #334155; vertical-align: top; width: 18%; white-space: nowrap;">${formatCurrency(getEffectiveUnitPrice(item))}</td>
      <td style="padding: 8px 6px; text-align: center; color: #334155; vertical-align: top; width: 12%;">${escapeHtml(item.quantity || 0)}</td>
      <td style="padding: 8px 6px; text-align: right; font-weight: 700; color: #0f172a; vertical-align: top; width: 26%;">${formatCurrency(item.lineTotal ?? (getEffectiveUnitPrice(item) * Number(item.quantity || 0)))}</td>
    </tr>
  `).join('');
}
      ...
      <div style="padding: 10px 14px 12px 14px;">
        <table style="width: 100%; border-collapse: collapse; font-size: 12px; table-layout: fixed;">
          <thead style="border-bottom: 1px solid #e2e8f0;">
            <tr>
              <th style="text-align: left; padding: 8px 6px; color: #64748b; font-weight: 700; width: 44%;">Item</th>
              <th style="text-align: right; padding: 8px 6px; color: #64748b; font-weight: 700; width: 18%;">Unit Price</th>
              <th style="text-align: center; padding: 8px 6px; color: #64748b; font-weight: 700; width: 12%;">Qty</th>
              <th style="text-align: right; padding: 8px 6px; color: #64748b; font-weight: 700; width: 26%;">Total</th>
            </tr>
          </thead>
          <tbody>
            ${renderItemRows(items)}
            <tr style="border-top: 1px solid #e2e8f0;">
              <td colspan="4" style="padding: 10px 6px 0 6px;">
                <div style="display: flex; justify-content: space-between; align-items: center; width: 100%;">
                  <div style="display: flex; gap: 16px; font-size: 11px; font-weight: 700; color: #64748b; text-transform: uppercase;">
                    <span>Items: <span style="color: #0f172a;">${totalItems}</span></span>
                    <span>Total Qty: <span style="color: #0f172a;">${totalQuantity}</span></span>
                  </div>
                  <div style="text-align: right;">
                    <span style="font-weight: 700; color: #64748b; font-size: 11px; text-transform: uppercase; margin-right: 8px;">Order Total:</span>
                    <span style="font-weight: 800; color: #0f172a; font-size: 13px;">${orderTotal}</span>
                  </div>
                </div>
              </td>
            </tr>
